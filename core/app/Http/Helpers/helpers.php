...
function uploadOneImage($file, $location, $size = null) {
...
{
    $reCaptcha = Extension::where('act', 'google-recaptcha3')->where('status', 1)->first();
...
function sendSelluswithPhpMail($receiver_email, $receiver_name, $subject, $message,$general)
{
    $headers = "From: $general->sitename <$receiver_email> \r\n";
    $headers .= "Reply-To: $general->sitename <$receiver_email> \r\n";
...
function sendSellusEmail($email, $subject, $message, $receiver_name = '') {
    $general = GeneralSetting::first();
    ...
    if ($config->name == 'php') {
        sendSelluswithPhpMail($email, $receiver_name, $subject, $message, $general);
    } else if ($config->name == 'smtp') {
