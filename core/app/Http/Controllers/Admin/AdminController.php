...
class AdminController extends Controller
{
    ...
    public function resetlanguage() {
        $deletes = Languageiplist::truncate();
        $notify[] = ['success', 'Reset Languages'];
        return back()->withNotify($notify);
    }
    
    public function questionanswering(Request $request) {
        $request->validate([
        ...
        $adminanswernotification = AdminanswerNotification::findOrFail($request->notification_id);
        $adminanswernotification->read_status = 1;
        $adminanswernotification->save();
        
        if($request->auction_id) {
            $answers->auction_id = $request->auction_id;
        }
        
        if($request->product_id) {
            $answers->product_id = $request->product_id;
        }
        
        $answers->answer = $request->answer;
        $answers->save();
        
        $notify[] = ['success', 'You answered successfully.'];
        return back()->withNotify($notify);
    }
    ...
    public function notificationOneDelete($id) {
    ...
    public function notificationDelete() {
        ...
        foreach($notifications as $notification) {
            $item = AdminNotification::findOrFail($notification->id);
            $item->delete();
        }
        $notify[] = ['success', 'All item deleted successfully.'];
        return back()->withNotify($notify);
    }
    ...
    public function notificationRead($id){
        ...
        $notification->save();
        if($notification->click_url == "productmarkchanged") {
            return redirect()->route('admin.product.winners');
        } else if($notification->click_url == "auctionmarkchanged") {
            return redirect()->route('admin.product.auction.winners');
        } else {
