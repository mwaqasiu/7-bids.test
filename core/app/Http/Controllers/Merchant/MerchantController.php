...
class MerchantController extends Controller
{
    public function dashboard()
    {
        ...
        foreach($user->bids()->groupBy('product_id')->get() as $key => $value) {
            $total_highest_amount = $total_highest_amount + $user->bids()->where('product_id', $value->product_id)->max('amount');
        }
        
        $widget['total_bid_amounts'] = $total_highest_amount;
    ...
    public function questionanswering(Request $request) {
        ...
        $sellernotification = SellerNotification::findOrFail($request->notification_id);
        $sellernotification->read_status = 1;
    ...
    public function closeAccount() {
        ...
        return view('merchant.closeaccount', compact('pageTitle', 'emptyMessage', 'widget'));
    }
    
    public function closeAccountclose() {
        $user = auth()->guard('merchant')->user();
        $users = Merchant::findOrFail($user->id);
        $users->status = 0;
        $users->save();
        $notify[] = ['success', 'Close Account successfully.'];
        return back()->withNotify($notify);
    }
    ...
    public function profileUpdate(Request $request)
    {
        ...
        $merchant->shopname= $request->shopname;
