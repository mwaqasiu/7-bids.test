                                            ...
                                            @if ($product->specification)
                                                @foreach ($product->specification as $spec)
                                                    @if ($spec['name'] == "Condition")
                                                        ...
                                                        @else
                                                            <!--<i class="fas fa-gem icon_select" title="Excellent Condition"></i>-->
                                                    ...
                                                    @elseif ($spec['name'] == "Certificated")
                                                        @if ($spec['value'] == "" && $spec['value'] == null)
                                                        @else
                                                            <span title="Certificated" class="icon_select">
                                                    ...
                                                    @elseif ($spec['name'] == "Literature")
                                                        @if ($spec['value'] == "" && $spec['value'] == null)
                                                            <!--<i class="fas fa-book-open" title="Mentioned in Literature"></i>-->
                                                        @else
                                                            <i class="fas fa-book-open icon_select" title="Mentioned in Literature"></i>
                                                        @endif
                                                    @elseif ($spec['name'] == "Edition")
                                                        @if ($spec['value'] == "" && $spec['value'] == null)
                                                        @else
                                                            <span title="Limited Edition" class="icon_select">
                                                    ...
                                                    @elseif ($spec['name'] == "Provenance")
                                                        @if ($spec['value'] == "" && $spec['value'] == null)
                                                            <!--<i class="fas fa-crown" title="Noteworthy Provenance"></i>-->
                                                        @else
                                                            <i class="fas fa-crown icon_select" title="Noteworthy Provenance"></i>
                                                        @endif
                                                    @endif
                                                @endforeach
                                            @endif
                                        </div>
                                        <div style="margin-left: 10px; min-height: 250px; display: flex; justify-content: center; align-items: center;">
                                ...
                                <div class="product-content">
                                    ...
                                    <div class="product-price" style="display: flex; justify-content: space-between; align-items: center;">
                                        ...
                                        <div style="margin-right: 10%;">
                                            <button class="cmn--btn btn--sm" data-cur_sym="€" style="margin: 0; font-weight: 100;">@lang('Buy it now')</button>
                                        </div>
                                    </div>
                                    @if ($product->status == 1 && $product->started_at < now() && $product->expired_at > now())
                                        <div class="btn__area">
                                            ...
                                            <div>
                                                <button class="cmn--btn btn--sm bid_now" data-cur_sym="{{ $general->cur_sym }}">@lang('Make an offer')</button>
                                            </div>
                                            <span class="text--danger empty-message">@lang('Please enter an amount to bid')</span>
                                        </div>
                                        <div style="text-transform: uppercase; text-align: center; margin-top: 3rem;">
                                            @if (auth()->check())
                                            <a class="btn-addshopcart" href="{{ route('user.shopping-cart.add', [$product->id, auth()->user()->id, getenv('REMOTE_ADDR')]) }}" title="Add to shopping cart">
                                                Add to shopping cart
                                            </a>
                                            @else
                                            <a class="btn-addshopcart" href="{{ route('user.shopping-cart.add', [$product->id, 'empty', getenv('REMOTE_ADDR')]) }}"  title="Add to shopping cart">
                                                Add to shopping cart
                                            </a>
                                            @endif
                                        </div>
            ...
            <div class="modal-dialog modal_image_dialog_big" style="max-width: 80%; width: fit-content; height: calc(100vh - 4px); display: flex; justify-content: center; align-items: center; margin: auto;" role="document">
                <div class="modal-content modal_image_content_big" style="flex-direction: row; background-color: rgba(0, 0, 0, 0.8); /* box-shadow: 0px 0px 2px 2px rgba(255, 255, 255, 0.3); */">
                    ...
                    <div class="modal_img_list">
                        <button class="btn text--danger modal-close high_img_modal_close" data-bs-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true" style="font-size: 30px;">&times;</span>
                            <span aria-hidden="true" style="font-size: 16px;">Close Window</span>
                        </button>
                        @if ($product->imagereplaceinput)
                            @foreach ($product->imagereplaceinput as $imgri)
                        ...
                        <div class="selected_img_download">
                            <a class="a_download_img" target="_blank" download rel="nofollow" title="Download photo" href="#">
                                <span style="font-size: 16px; color: #ea5455;">Download Photo</span>
...
@push('style')
    <style>
        ...
        .img_scale_slider > .slider::-webkit-slider-thumb {
            ...
            border-radius: 50%;
            /*background: url('https://1400g.de/assets/images/controller.png');*/
            cursor: pointer;
        }
