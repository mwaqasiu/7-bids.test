...
<div class="d-flex flex-wrap justify-content-sm-between justify-content-end mb-4" style="gap:15px 30px;background: transparent !important;">
    <p class="mb-0">
        <!--@lang('Showing Results'): <span>{{ $auctions->count() }}</span>-->
    </p>
                ...
                <div class="auction__item-thumb">
                    <a href="{{ route('auction.details', [$auction->id, slug($auction->name)]) }}" style="position: relative;">
                        ...
                        <div class="item-image-tags{{ $auction->id }}" style="height: 250px;">
                            <img style="width: 100%; height: 100%;" src="{{getImage(imagePath()['product']['path'].'/thumb_'.$auction->image,imagePath()['product']['thumb'])}}" alt="auction">
                        </div>
                            ...
                            <table>
                                @if($auction->specification)
                                    @foreach ($auction->specification as $spec)
                                        @if($loop->index == 0)
                                        @elseif($loop->index < 7 && $loop->index > 0)
                                            @if($spec['value'] != null)
                                                <tr>
                                                    <th>{{ __($spec['name']) }} :</th>
                                                    <td>{{ __($spec['value']) }}</td>
                    ...
                    <div class="item_provenance_icon">
                        @if (auth()->check())
                            ...
                            @endforeach
                            @if($wishnum > 0)
                                <a class="item_heart_icon_red" href="{{ route('user.auctionwishlist.onedelete', [$wishid]) }}" title="@lang('Add to wish list')">
                            ...
                            <div class="total-price">
                                @if($winnertext == "winner")
                                    <span class="productcost">
                                        @lang('Sold for'): {{ $general->cur_sym }} {{ showAmount($auction->price, 0) }} (@lang('excl'). @lang('fees'))
                                    </span>
                                @else
                                    <span class="productcost">
...
<script>
    (function ($) {
        ...
        $('.item_shift_icon').on('click', function() {
            ...
            var itemtags = "item_shift_icon_replace" + data.auctionid;
            var origintags = "item_shift_icon" + data.auctionid;
            $('.' + itemtags).css('display', 'flex');
            var blocktags = "item-block-tags" + data.auctionid;
            var imagetags = "item-image-tags" + data.auctionid;
        ...
        $('.item_shift_icon_replace').on('click', function() {
            var data = $(this).data();
            $(this).css('display', 'none');
            var itemtags = "item_shift_icon" + data.auctionid;
            $('.' + itemtags).css('display', 'flex');
            var blocktags = "item-block-tags" + data.auctionid;
            var imagetags = "item-image-tags" + data.auctionid;
            $('.' + blocktags).css("display", 'none');
