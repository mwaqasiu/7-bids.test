            ...
            <table class="table table--light style--two">
                ...
                <tbody>
                    @forelse ($winningHistories as $winner)     
                        <tr>
                            <td data-label="@lang('Marketplace Item')" class="tdfordesktop">{{ __($winner->product->name) }}</td>
                            <td data-label="{{ __($winner->product->name) }}" class="tdformobile">&nbsp;</td>
                            <td data-label="@lang('Price')">
                                {{ $general->cur_sym }} {{ showAmount($winner->bid->amount, 0) }}
                            </td>
                            <td data-label="@lang('Status')">
                                ...
                                @elseif($winner->product_delivered == 1)
                                    <div class="status-main">
                                        ...
                                        <div class="pickedstatusinactive" title="@lang('View Progress')" data-bidid="{{ $winner->id }}">@lang('Picked')</div>
                                        <div class="status-stick"></div>
                                        <div class="packedstatusinactive" title="@lang('Packing Report')" data-bidid="{{ $winner->id }}">@lang('Packed')</div>
                                        <div class="status-stick"></div>
                                ...
                                @elseif($winner->product_delivered == 2)
                                    <div class="status-main">
                                        <div class="paidstatus" title="@lang('View Invoice')" data-bidid="{{ $winner->id }}">@lang('Paid')</div>
                                        <div class="status-stick"></div>
                                        <div class="pickedstatus" title="@lang('View Progress')" data-bidid="{{ $winner->id }}">@lang('Picked')</div>
                                        <div class="status-stick"></div>
                                        <div class="packedstatusinactive" title="@lang('Packing Report')" data-bidid="{{ $winner->id }}">@lang('Packed')</div>
                                        <div class="status-stick"></div>
                                        <div class="transitstatusinactive" title="@lang('Track & Trace')">@lang('Shipped')</div>
                                ...
                                @elseif($winner->product_delivered == 3)
                                    <div class="status-main">
                                        ...
                                        <div class="pickedstatus" title="@lang('View Progress')" data-bidid="{{ $winner->id }}">@lang('Picked')</div>
                                        <div class="status-stick"></div>
                                        <div class="packedstatus" title="@lang('Packing Report')" data-bidid="{{ $winner->id }}">@lang('Packed')</div>
                                        <div class="status-stick"></div>
                                        <div class="transitstatusinactive" title="@lang('Track & Trace')">@lang('Shipped')</div>
                                    </div>
                                @else
                                    <div class="status-main">
                                        ...
                                        @else
                                            <div class="transitstatus" title="@lang('Track & Trace')">
                                                <a href="{{ $url }}" class="getformatag" target="_blank">
                                                    @lang('Shipped')
                                                </a>
                                            </div>
                                        @endif
                            ...
                            <td data-label="@lang('Purchase Date')">
                                {{ showDateTime($winner->created_at) }} <br>
                                <span class="subpurchasedatespan">{{ diffForHumans($winner->created_at) }}</span>
                            </td>
                            <td data-label="@lang('Further Actions')" style="display: flex; justify-content: flex-end;">
                                @if($winner->product_delivered == 0)
                                    @if(trim($winner->pending_imageurl) == "")
                                        <button style="width: 28px; margin-right: 5px;" data-existimage="" data-bidid="{{ $winner->id }}" class="icon-btn generalsysviewbtn" title="@lang('View Invoice')">
                                            €
                                        </button>
                                    @else
                                ...
                                <a href="{{ route('product.details', [$winner->product->id, slug($winner->product->name)]) }}" target="__blank"
                                    class="icon-btn bid-details bid-details-tag" style="width: 28px; margin-right: 5px;" title="@lang('View Marketplace Lot')">
                                    <i class="las la-eye text--shadow"></i>
                                </a>
                                
                                @if($winner->product_delivered == 4)
                                    <button style="width: 28px;" data-delid="{{ $winner->id }}" class="icon-btn btn--danger biddelbtn biddelbtntag" >
                                        <i class="las la-trash text--shadow"></i>
                                    </button>
                                @else
                                    <button style="width: 28px;" data-delid="{{ $winner->id }}" class="icon-btn btn--danger biddelbtntag" disabled>
                                        <i class="las la-trash text--shadow"></i>
                                    </button>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td class="text-muted text-center table-empty-td" style="padding-left: 0px !important; padding-right: 0px !important;" colspan="100%">{{ __($emptyMessage) }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="modal-content modal_image_content_big" style="flex-direction: row; background-color: rgba(0, 0, 0, 0.8); /* box-shadow: 0px 0px 2px 2px rgba(255, 255, 255, 0.3); */">
            ...
            <div class="modal_img_list" style="background: transparent; position: relative;">
                ...
                <button class="btn text--danger close high_img_modal_close" data-dismiss="modal" aria-label="Close" title="@lang('Close Window')">
                    <span aria-hidden="true" style="font-size: 20px; background: transparent; text-shadow: none; font-weight: 400; z-index: 100; padding: 1px 4px;">&times;</span>
                </button>
...
<div id="deleteModal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('user.winninghistory.delete') }}" method="POST">
                @csrf
...
<div id="fastshippingModal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('user.winningbid.productupdatecheckout') }}" method="POST">
                @csrf
                <div class="modal-body">
...
<div id="fastaddshippingModal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn--primary shippingaddressaddbtn">@lang('Add')</button>
            </div>
        </div>
    </div>
</div>
...
<div id="fastpickupdateModal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('user.winningbid.productupdatepickupdate') }}" method="POST">
                @csrf
                <div class="modal-body">
...
<div id="fastpi" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-body">
                <form action="{{ route('user.winningbid.productunmarkpickupdate') }}" method="POST">
                    @csrf
...
<div id="aaa" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-body">
                <form action="{{ route('user.winningbid.productunupdatecheckout') }}" method="POST">
                    @csrf
...
<div id="fastcombinedshippingModal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('user.winningbid.productupdatecombineshipping') }}" method="POST">
                @csrf
                <div class="modal-body">
...
<div id="fastcom" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-body">
                <form action="{{ route('user.winningbid.productunmarkcombineshipping') }}" method="POST">
                    @csrf
...
<script>
    ...
    (function ($) {
        ...
        $(".searchSel").on("change", function() {
            if($(this).val() == "120") {
                window.location.href = "{{ route('user.winning.history') }}";
            } else {
                window.location.href = "{{ route('user.winning.history') }}/search/"+$(this).val();
            }
        });
        ...
        $('.generalsysviewbtn').on('click', function() {
            ...
            if(data.existimage == "") {
                ...
                var winningbidsdataary = @php echo $winningHistories; @endphp;
