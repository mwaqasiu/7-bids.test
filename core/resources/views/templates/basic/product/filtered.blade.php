        ...
        <div class="col-sm-6 col-xl-4">
            <div class="auction__item bg--body">
                <div class="auction__item-thumb">
                    ...
                    @endif
                    <button class="item_shift_icon_replace item_shift_icon_replace{{ $product->id }}" type="button" data-productid="{{ $product->id }}">
                        &nbsp;
                    </button>
                    ...
                    @endif
                    <div class="item-image-tags{{ $product->id }}" style="border: 1px solid transparent; display: flex; position: relative; width: calc(100%); padding-top: calc(100%); overflow: hidden;">
                        ...
                        @if($product->imagereplaceinput)
                            @foreach ($product->imagereplaceinput as $imgri)
                                ...
                                @if($imagereplaceinputnumber <= 1)
                                    <img class="item-slider-replace-image-view{{$product->id}}" style="width: 100%; height: 100%; position: absolute; top: 0; right: 0; left: 0; bottom: 0; object-fit: fill;" src="{{getImage(imagePath()['product']['path'].'/'.$imgri['url'], imagePath()['product']['size'])}}" alt="auction">
                                @endif
                            @endforeach
                        @else
                            <img class="item-slider-replace-image-view{{$product->id}}" style="width: 100%; height: 100%; position: absolute; top: 0; right: 0; left: 0; bottom: 0; object-fit: fill;" src="{{getImage(imagePath()['product']['path'].'/thumb_'.$product->image, imagePath()['product']['size'])}}" alt="auction">
                        @endif
                    </div>
                    <div class="item-block-tags item-block-tags{{ $product->id }}" style="position: relative; width: calc(100%); padding-top: calc(100%); overflow: hidden;">
                        <div style="position: absolute; top: 0; bottom: 0; left: 0; right: 0; padding: 12px 20px; justify-content: center; align-items: center; display: flex;">
                            <table class="item-block-tables" style="width: 100%;">
                ...
                <div class="auction__item-slider">
                    <div class="auction__item-subslider">
                        @if ($product->imagereplaceinput)
                            @foreach ($product->imagereplaceinput as $imgri)
                                <div class="auction__slider-btn" data-productid="{{ $product->id }}" data-imagesrc="{{getImage(imagePath()['product']['path'].'/'.$imgri['url'],imagePath()['product']['size'])}}"></div>
                            @endforeach
                        @endif
                    </div>
                </div>
                <div class="auction__item-content">
                    <div class="item_provenance_icon">
                        ...
                        @if (auth()->check())
                            @foreach($wishlists as $wishlist)
                                @if($wishlist->product_id == $product->id && $wishlist->ip_address == getenv('REMOTE_ADDR'))
                                    @php
                                        $wishnum = $wishnum + 1;
                                        $wishid = $wishlist->id;
                                    @endphp
                            ...
                            @endforeach
                            @if($wishnum > 0)
                                <a style="cursor: pointer" class="item_heart_icon_red" href="{{ route('user.wishlist.onedelete', [$wishid]) }}" title="@lang('Delete from wish list')">
                                    <span><i class="lar la-heart"></i></span>
                                </a>
                            @else
                                <a style="cursor: pointer" class="item_heart_icon" href="{{ route('user.wishlist.add', [$product->id, auth()->user()->id, getenv('REMOTE_ADDR')]) }}" title="@lang('Add to wish list')">
                                    <span><i class="lar la-heart"></i></span>
                                </a>
                            @endif
                        @else
                            ...
                            @else
                                <a style="cursor: pointer" class="item_heart_icon" href="{{ route('user.wishlist.add', [$product->id, 'empty', getenv('REMOTE_ADDR')]) }}" title="@lang('Add to wish list')">
                                    <span><i class="lar la-heart"></i></span>
                                </a>
                            @endif
                        @endif
                    ...
                    <h6 class="auction__item-title" style="height: 65px; display: flex; align-items: flex-start; justify-content: center;">
                        @if(isset($product->winner))
                            <span style='font-size: 15px; text-align: center;'>{{ __($product->name) }}</span>
                        @else
                            <a href="{{ route('product.details', [$product->id, slug($product->name)]) }}" style='font-size: 15px; text-align: center;'>{{ __($product->name) }}</a>
                        @endif
                    </h6>
                            ...
                            <div class="total-price">
                                @if($product->winner)
                                    <span class="productcost">
                                        @lang('Sold for'): {{ $general->cur_sym }} {{ showAmount($product->winner->bid->amount, 0) }}
                                    </span>
                                @else
                                    <span class="productcost">
                                        {{ $general->cur_sym }} {{ showAmount($product->price, 0) }}
                                    </span>
                                @endif
                            </div>
                            @if($numberDays >= 29)
...
@push('script')
    <script>
        ...
        $('.auction__slider-btn').on('click', function() {
            const data = $(this).data();
            $('.item-slider-replace-image-view' + data.productid).attr("src", data.imagesrc);
        });
    </script>
@endpush
