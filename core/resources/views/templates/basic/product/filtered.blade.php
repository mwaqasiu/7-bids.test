...
<div class="d-flex flex-wrap justify-content-sm-between justify-content-end mb-4" style="gap:15px 30px;background: transparent !important;">
    <p class="mb-0">
        <!--@lang('Showing Results'): <span>{{ $products->count() }}</span>-->
    </p>
                ...
                <div class="auction__item-content">
                    ...
                    <div class="item_provenance_icon">
                        @if (auth()->check())
                            ...
                            @endforeach
                            @if($wishnum > 0)
                                <a class="item_heart_icon_red" href="{{ route('user.wishlist.onedelete', [$wishid]) }}" title="@lang('Add to wish list')">
                            ...
                            <div class="total-price">
                                @if($winnertext == "winner")
                                    <span class="productcost">
                                        @lang('Sold for'): {{ $general->cur_sym }} {{ showAmount($product->soldamount, 0) }}
                                    </span>
                                @else
                                    <span class="productcost">
...
<script>
    (function ($) {
        ...
        $('.item_shift_icon').on('click', function() {
            ...
            var itemtags = "item_shift_icon_replace" + data.productid;
            var origintags = "item_shift_icon" + data.productid;
            $('.' + itemtags).css('display', 'flex');
            var blocktags = "item-block-tags" + data.productid;
            var imagetags = "item-image-tags" + data.productid;
            var newpricetags = "newpricecalc" + data.productid;
        ...
        $('.item_shift_icon_replace').on('click', function() {
            var data = $(this).data();
            $(this).css('display', 'none');
            var itemtags = "item_shift_icon" + data.productid;
            $('.' + itemtags).css('display', 'flex');
            var blocktags = "item-block-tags" + data.productid;
            var imagetags = "item-image-tags" + data.productid;
            var newpricetags = "newpricecalc" + data.productid;
