    ...
    <div class="col-lg-12">
        <div class="card b-radius--10 ">
            <div class="card-body p-0">
                <div class="pageflagsection">
                    <div class="form-group">
                        <label>@lang('Admin')</label>
                        <div>
                            <label class="switch">
                                <input type="checkbox" name="pageflag" id="pageflag">
                ...
                <div class="table-responsive--md  table-responsive">
                    <table class="table table--light style--two">
                        ...
                        @forelse($winners as $winner)
                            <tr class="{{ $winner->product->admin_id ? 'admintritem' : 'merchanttritem' }}">
                                <td data-label="@lang('S.N')">
                                    @if($winner->product->admin_id)
                                    {{ substr($winner->product->created_at, 0, 4) }}-0-M-{{ $winner->product->id }}
                                    @else
                                    {{ substr($winner->product->created_at, 0, 4) }}-{{ $winner->product->merchant_id }}-M-{{ $winner->product->id }}
                                    @endif
                                </td>
...
<div id="winnerdeleteModal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{route('admin.product.winnerdelete')}}" method="POST">
                @csrf
                <div class="modal-body">
...
<style>
    ...
    .pageflagsection > div > label {
        font-size: 14px;
        font-weight: bold;
    }
    
    .pageflagsection > div > label:first-child {
        margin: 0;
        margin-right: 3px;
    }
    
    .pageflagsection > div > label:last-child {
        margin: 0;
        margin-left: 3px;
    }
...
@push('script')
    <script>
        (function ($) {
            ...
            $('#pageflag').on('change', function() {
                if($('#pageflag').is(":checked")) {
                    $('.merchanttritem').css('display', 'table-row');
                    $('.admintritem').css('display', 'none');
                } else {
                    $('.merchanttritem').css('display', 'none');
                    $('.admintritem').css('display', 'table-row');
                }
            });
